...
const ToastViewport = React.forwardRef(({ className, ...props }, ref) => (
  <ToastPrimitives.Viewport
    ref={ref}
    className={cn(
      "fixed bottom-4 left-4 right-4 z-[100] flex max-h-screen flex-col items-end gap-2 sm:left-auto sm:right-4 sm:max-w-[360px]",
      className
    )}
    {...props}
  />
...
const stripToastPositioning = (value) => {
  if (typeof value !== "string") return value;
  return value
    .replace(/\b(fixed|bottom-12|left-1\/2|transform|-translate-x-1\/2|z-50)\b/g, "")
    .replace(/\s{2,}/g, " ")
    .trim();
};
...
const Toast = React.forwardRef(({ className, variant, ...props }, ref) => {
  return (
    <ToastPrimitives.Root
      ref={ref}
      className={cn(toastVariants({ variant }), stripToastPositioning(className))}
      {...props}
    />
  );
});
...
const ToastAction = React.forwardRef(({ className, ...props }, ref) => (
  <ToastPrimitives.Action
    ref={ref}
    className={cn(
      "inline-flex h-8 shrink-0 items-center justify-center rounded-md border bg-transparent px-3 text-sm font-medium transition-colors hover:bg-secondary focus:outline-none focus:ring-1 focus:ring-ring disabled:pointer-events-none disabled:opacity-50 group-[.destructive]:border-muted/40 group-[.destructive]:hover:border-destructive/30 group-[.destructive]:hover:bg-destructive group-[.destructive]:hover:text-destructive-foreground group-[.destructive]:focus:ring-destructive group-[.success]:border-green-200/40 group-[.success]:hover:border-green-300/50 group-[.success]:hover:bg-green-500 group-[.success]:hover:text-white group-[.success]:focus:ring-green-400",
      className
    )}
    {...props}
  />
